...
// Embedded JS statements (JS_BLOCK) — e.g. Quark's @id directives —
// run here, after OAS_OBJ is built but before the scene starts, so
// document/Quark calls execute in source order relative to scene setup
// without depending on scene internals.
`,
    },
    {
...
currentANIM = ObjectAnimationSystem_INS.main(OAS_OBJ).init(60, true);`,
  ...
  function reshapeToInps(PST) {
    ...
    let inputs = {
      ...
      errorsLOGS: [],
      scenes: [],
      // Raw embedded JS statements (e.g. Quark's @id directives, or any
      // other plugin/expression that compiles to a JS_BLOCK node). These
      // aren't scene data, so they don't go through scenesSpecificInputs
      // hoisting — they're collected in file order and emitted once,
      // right before the scene is pushed to the render pipeline. Without
      // this key, `inputs[r[j].type]` below would be undefined for
      // "JS_BLOCK" and the statement would be silently dropped from
      // final output — this was a real, verified bug (see
      // JS_BLOCK-dropped fix note in postSemanticAnalyizer.js) affecting
      // every JS_BLOCK-producing plugin, not just Quark.
      JS_BLOCK: [],
    };
