...
Only relevant for packages with `"type": "plugin"` in their `cdrca.json`.
This is CDRCA's real, existing plugin security model (from CDRCA's own
`plugin.js`) — the CLI surfaces it, it doesn't invent it.
